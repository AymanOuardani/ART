...
#Les 30 canaux du template ART, dans l'ordre
...
parser.add_argument("Signal", choices=["brut", "iclabel"],
                    help="brut : le signal d'origine | iclabel : sa version nettoyée (ICLABEL_Brut.py)")
parser.add_argument("Sujet", type=int, help="Numéro du sujet (1-109)")
args = parser.parse_args()
sujet_id = "S" + str(args.Sujet).zfill(3)

#Le brut et sa version ICLabel passent par le même traitement, sans quoi ils ne seraient pas
#comparables échantillon par échantillon.
source = (Brut_fif_ICLABEL if args.Signal == "iclabel" else Brut_fif) / (sujet_id + "-raw.fif")
if not source.exists():
    raise SystemExit(f"ERREUR : fichier introuvable : {source}"
                     + ("\n  (lance d'abord ICLABEL_Brut.py)" if args.Signal == "iclabel" else ""))

#Lecture et standardisation des noms de canaux
raw = mne.io.read_raw_fif(source, preload=True)
eegbci.standardize(raw)

#Sélection et réordonnancement des 30 canaux
...
#Tout le signal : les deux mouvements et le repos
...
out = (Nettoye / sujet_id / "ICLABEL.fif") if args.Signal == "iclabel" else (Pretraite / (sujet_id + "_Pre.fif"))

#Blocs de 4 s autour des événements
X, y = [], []

"""
Pretraitement EEGBCI -> format ART (art. 2.3), pour le motor imagery gauche/droite.
...
Puis decoupe en essais de 4 s (1024 pts) autour de T1 (gauche) / T2 (droite).
    ...
    python Pretreatement.py --subjects 1-109            # un fichier par sujet
    python Pretreatement.py --subjects 1                # test rapide

Sortie : Code/Prétraité/S001-epo.fif, S002-epo.fif, ... (objets MNE Epochs).
"""
...
OUT_DIR = Path(__file__).resolve().parent / "Prétraité"   # sorties pretraitees ici
...
def load_raw(subject):
    # Telecharge (dans Code/Brut) et concatene les runs 4/8/12 d'un sujet
...
def save_subject(X, y, subject):
    # Ecrit un fichier par sujet dans Code/Prétraité (S001-epo.fif)
    out = OUT_DIR / f"S{subject:03d}-epo.fif"
    to_epochs(X, y, np.full(len(y), subject)).save(out, overwrite=True)
    return out


def main():
    ap = argparse.ArgumentParser(description="Pretraitement EEGBCI -> format ART.")
    ap.add_argument("--subjects", default="1", help="ex. 1-109 ou 1,2,5")
    args = ap.parse_args()

...
bruit a SNR variable). Le modele mono-canal = make_model(src=1, tgt=1, N).
    ...
    python Training_ART_CPU.py                        # bruit Hybrid, 50 epochs
    python Training_ART_CPU.py --noise EOG --epochs 30
    python Training_ART_CPU.py --noise EMG --batch_size 8
...
# --------------------------------- Main ------------------------------------
def main():
    ap = argparse.ArgumentParser(description="Entrainement ART sur EEGdenoiseNet (CPU).")

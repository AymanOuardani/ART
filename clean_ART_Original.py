...
parser = ap.ArgumentParser(description="Débruitage ART (fidèle au dépôt original)")
...
data = epochs.get_data()                        # (n_essais, 30, 1024)
data_clean = np.empty_like(data)
for i in range(len(data)):
    data_noise = data[i]

    # normalisation SCALAIRE (comme reconstruct() de utils.py, lignes 239-242)
    std = np.std(data_noise)
    avg = np.average(data_noise)
    data_noise = (data_noise - avg) / std

    # décodeur ART (copié tel quel de decode_data(), branche ART)
    with torch.no_grad():
    ...
    data_clean[i] = decode[0]   # sortie gardée normalisée, SANS re-scale (comme eux)
...
print("Signaux nettoyés (ART original) sauvegardés pour Sujet ", args.Sujet)

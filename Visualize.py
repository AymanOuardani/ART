...
parser.add_argument("Signal", choices=["brut", "pretraite", "ART", "ICUNet", "ICA"],
...
if args.Signal == "brut":
    ...
    if args.Signal == "ICA":
        fichier_apres = Nettoye / sujet_id / (sujet_id + "-ICA.fif")
    else:
        fichier_apres = Nettoye / sujet_id / fichiers_apres[args.Signal]
    avant = mne.read_epochs(Pretraite / (sujet_id + "-epo.fif"), preload=True)
